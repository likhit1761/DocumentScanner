...
def uploadimages():
    ...
    for file in files_in_dir:
        if file[0] != '.' and file[0] != '_':
            if file not in ['static','README.md', 'templates', 'v1_templates', 'app.py', 'Procfile', 'requirements.txt', 'venv']:
                os.remove(file)
...
def uploadimage():
    try:
        ...
        for file in files_in_dir:
            if file[0] != '.' and file[0] != '_':
                if file not in ['static','README.md', 'templates', 'app.py', 'Procfile', 'v1_templates', 'requirements.txt',
                                'venv']:
                    os.remove(file)
        uploaded_files = request.files.getlist("files")
        for file in uploaded_files:
...
def uploadspeech():
    try:
        ...
        for file in files_in_dir:
            if file[0] != '.' and file[0] != '_':
                if file not in ['static','README.md', 'templates', 'v1_templates', 'app.py', 'Procfile', 'requirements.txt',
                                'venv']:
                    os.remove(file)
        uploaded_files = request.files.getlist("files")
        for file in uploaded_files:
